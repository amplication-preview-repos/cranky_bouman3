...
export class StudentServiceBase {
  ...
  async findAcceptanceLetters(
    parentId: string,
    args: Prisma.AcceptanceLetterFindManyArgs
  ): Promise<PrismaAcceptanceLetter[]> {
    return this.prisma.student
      .findUniqueOrThrow({
        where: { id: parentId },
      })
      .acceptanceLetters(args);
  }

  async findAdmissionLetters(
    parentId: string,
    args: Prisma.AdmissionLetterFindManyArgs
  ): Promise<PrismaAdmissionLetter[]> {
    return this.prisma.student
      .findUniqueOrThrow({
        where: { id: parentId },
      })
      .admissionLetters(args);
  }
